...
## 2026-08-22 23:44（北京时间）— 新增 DOI 元数据并发预检
...
## 2026-08-22 23:50（北京时间）— 新增统一批次 API-only 模式
...
## 2026-08-23 20:05（北京时间）— 规范化项目变更记录
...
## 2026-08-23 20:13（北京时间）— 清理旧上游公开归属
...
## 2026-08-23 20:36（北京时间）— 精简 GitHub 首页并重组公开文档
...
## 2026-08-23 21:02（北京时间）— 删除旧交接输出与源码打包流程

- **任务目标：** 按用户确认彻底删除研究生交接输出功能、内部历史计划和 Windows UI 源码打包脚本，减少旧功能与公开仓库噪声。
- **新增、修改或删除的文件：** 删除 `student_handoff.py`、`tests/test_student_handoff.py`、`docs/superpowers/` 下 7 个历史计划/规格文件和 `scripts/build/make_windows_ui_package.bat`；修改 ScienceDirect 适配器、兼容 CLI、`RunSummary`、GUI、相关测试、中英文文档、兼容 Skill、`.gitignore` 和 `AGENTS.md`。
- **具体修改内容：** 新任务停止生成 `00_给研究生查看/`、`paper_index.csv/xlsx`、`失败项_下一步处理.csv` 和 `library_index.csv`；`run_summary.json` 删除 5 个交接路径字段；GUI 删除两个交接入口按钮；公开发布说明改为使用 GitHub Source code 压缩包或 `git clone`，不再提供自定义 Windows 源码包命令。
- **修改原因：** 这些交接与源码打包能力不再属于当前推荐产品路径；直接删除单个模块会破坏导入链，因此同步移除了调用、输出契约、界面、文档和测试。
- **生成的输出文件：** 未生成发布包、下载结果或实验数据；测试只在系统临时目录和忽略的缓存目录中生成短期文件。
- **如何检查是否成功：** `paper_batch.py --help`、`sd_institutional_skill.py --help` 和 `sd_scraper.py --help` 均正常；项目规定的 `compileall` 通过；受影响专项测试 106 项通过；完整离线测试 450 项通过、2 项因 Windows 符号链接权限跳过；最终检查目标路径不存在、活动代码与文档无残留引用，并运行 `git diff --check`。
- **注意事项或潜在风险：** 已有批次中的旧交接目录和索引不会删除或修改；依赖旧 5 个 JSON 字段的外部脚本需要更新；以后不再支持 `scripts/build/make_windows_ui_package.bat`。本次未自动打包、提交或推送 GitHub。

## 2026-08-23 21:04（北京时间）— 统一变更记录时区显示

- **任务目标：** 将项目变更记录中的时区显示改为更直观的北京时间。
- **新增、修改或删除的文件：** 仅修改 `CHANGELOG.md`；未新增或删除文件。
- **具体修改内容：** 将全部标题中的技术时区标签统一替换为“北京时间”，后续记录继续使用该显示方式。
- **修改原因：** 原标签是 UTC+8 的 IANA 技术标识，但面向中文用户时“北京时间”更容易理解。
- **生成的输出文件：** 更新后的 `CHANGELOG.md`。
- **如何检查是否成功：** 搜索确认所有记录统一显示“北京时间”，并运行 `git diff --check`。
- **注意事项或潜在风险：** 本次只修改显示文字，不改变时间数值、系统时区或程序行为；未自动提交或推送 GitHub。
