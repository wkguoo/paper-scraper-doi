...
5. 保持“检索后下载 PDF”勾选；默认会同时下载 ScienceDirect 补充材料，如只要正文 PDF，可取消“同时下载补充材料”。只有 PDF 下载启用且该复选框保持勾选时，才会生成补充材料报告和目录。如果不用 Cookie JSON，再按需选择“从本机浏览器读取 Cookie”或“先弹出浏览器手动登录”（默认优先 Chrome/Chromium；Edge 仅在你显式指定其路径时使用）。
...
该模式不会使用 Cookie JSON；常规下载失败后会自动回退到第三方数据源。
